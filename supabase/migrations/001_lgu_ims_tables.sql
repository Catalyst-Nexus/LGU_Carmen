...
CREATE TABLE IF NOT EXISTS hr.personnel (
  ...
  is_active         BOOLEAN     NOT NULL DEFAULT true,
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
  -- Photo — stores the storage path (e.g. "per_uuid/photo.jpg") in the
  -- personnel_photos bucket, NOT a full URL.  Generate a signed URL at display time.
...
-- =============================================================================
-- STORAGE BUCKETS
-- profile_picture  — system user avatars (auth.users metadata)
-- personnel_photos — HR personnel photos (hr.personnel.profile_photo_path)
-- =============================================================================
...
-- profile_picture policies
DROP POLICY IF EXISTS "profile_picture_select" ON storage.objects;
DROP POLICY IF EXISTS "profile_picture_insert" ON storage.objects;
DROP POLICY IF EXISTS "profile_picture_update" ON storage.objects;
DROP POLICY IF EXISTS "profile_picture_delete" ON storage.objects;

CREATE POLICY "profile_picture_select" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'profile_picture');
CREATE POLICY "profile_picture_insert" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'profile_picture');
CREATE POLICY "profile_picture_update" ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'profile_picture');
CREATE POLICY "profile_picture_delete" ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'profile_picture');

-- personnel_photos policies
DROP POLICY IF EXISTS "personnel_photos_select" ON storage.objects;
DROP POLICY IF EXISTS "personnel_photos_insert" ON storage.objects;
DROP POLICY IF EXISTS "personnel_photos_update" ON storage.objects;
DROP POLICY IF EXISTS "personnel_photos_delete" ON storage.objects;

CREATE POLICY "personnel_photos_select" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'personnel_photos');
CREATE POLICY "personnel_photos_insert" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'personnel_photos');
CREATE POLICY "personnel_photos_update" ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'personnel_photos');
